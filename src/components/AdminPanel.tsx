...
export default function AdminPanel({ fid }: AdminPanelProps) {
  ...
  const [activeTab, setActiveTab] = useState<'tokens' | 'config' | 'players' | 'reset'>('tokens')
  ...
  const handleDeleteToken = async (tokenId: number) => {
    if (!confirm('Are you sure you want to delete this token?\n\nWARNING:\n- All positions will be deleted\n- Open positions: collateral will be refunded automatically\n- Closed positions: historical data will be lost')) return
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.total_positions > 0) {
          message += `\n\nDeleted ${data.total_positions} position(s)`
          if (data.refunded_players > 0) {
            message += `\nRefunded ${data.refunded_players} player(s) (${data.closed_positions} open positions)`
          }
        }
              ...
              <form onSubmit={handleAddToken} className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <input
                    ...
                    className="bg-[#0a0c12] border border-gray-700 rounded-lg px-4 py-2 text-white w-full"
                    required
                  />
                  <input
                    type="text"
              ...
              {configs.map((config) => (
                ...
                >
                  ...
                  <input
                    ...
                    className="bg-[#090a0f] border border-gray-700 rounded-lg px-4 py-2 text-white w-32 max-w-full text-right font-mono"
                  />
                </div>
              ))}
            </div>
              ...
              <div className="bg-[#0a0c12] rounded-xl p-6 border border-gray-700/50">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <input
                    ...
                    className="flex-1 bg-[#090a0f] border border-gray-700 rounded-lg px-4 py-3 text-white text-xl font-bold max-w-full"
                  />
                </div>
                {affectedPlayers > 0 && (
              ...
              <div className="bg-[#0a0c12] rounded-xl p-6 border border-gray-700/50">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <input
                    ...
                    className="flex-1 bg-[#090a0f] border border-gray-700 rounded-lg px-4 py-3 text-white text-lg max-w-full"
                  />
                </div>
              </div>
